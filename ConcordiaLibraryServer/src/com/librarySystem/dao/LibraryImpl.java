...
public class LibraryImpl extends UnicastRemoteObject implements LibraryInterface {
	...
	public synchronized boolean addItem(String managerID, String itemID, String itemName, int quantity){
		...
		if (result == true) {
			...
			Utilities.serverLog(University.CONCORDIA.getCode(), "Addition of Item", managerID, "The item cannot be added to the inventory!!");
	...
	public synchronized ArrayList<Item> listItemAvailability(String managerID){
		...
		Utilities.serverLog(University.CONCORDIA.getCode(), "Requested List of Items", managerID, "The Items have been listed successfully");
	...
	public synchronized boolean borrowItem(String userID, String itemID, int numberOfDays){
		...
		if (result == true) {
			...
			Utilities.serverLog(University.CONCORDIA.getCode(), "Borrwing an Item", userID, "Item borrowed Successfully!!");
		} else {
			Utilities.clientLog(userID, "Borrwing an Item", "Item could not be borrowed!!");
			Utilities.serverLog(University.CONCORDIA.getCode(), "Borrwing an Item", userID, "Item could not be borrowed!!");
		}
		return result;
	}
	...
	public synchronized ArrayList<Item> findItem(String userID, String itemName){
		...
		if (ResultList.size() != 0) {
			...
			Utilities.serverLog(University.CONCORDIA.getCode(), "Finding an item", userID, "Items listed successfully!!");
		} else {
			Utilities.clientLog(userID, "Finding an item", "No items with the given name found!!");
			Utilities.serverLog(University.CONCORDIA.getCode(), "Finding an item", userID, "No items with the given name found!!");
		}
		return ResultList;
	}
	...
	public boolean returnItem(String userID, String itemID){
		...
		if (result == true) {
			...
			Utilities.serverLog(University.CONCORDIA.getCode(), "Returning an Item", userID, "The item was successfully returned!!");
		} else {
			Utilities.clientLog(userID, "Returning an Item", "The item cannot be returned!!");
			Utilities.serverLog(University.CONCORDIA.getCode(), "Returning an Item", userID, "The item cannot be returned!!");
		}
		return result;
	}
	...
	public boolean addToQueue(String itemID, String userID){
		...
		if (result == true) {
			...
			Utilities.serverLog(University.CONCORDIA.getCode(), "Wait queue", userID, "The user was successfully added to the wait queue!!");
		} else {
			Utilities.clientLog(userID, "Wait queue", "The user cannot be added to the wait queue!!");
			Utilities.serverLog(University.CONCORDIA.getCode(), "Wait queue", userID, "The user cannot be added to the wait queue!!");
		}
		return result;
	}
